...
docker:
	docker buildx build --build-arg='BUILD_TAGS=mainnet,experimental' -t tztcloud/go-livepeer:grant-node-0.7.5 -f docker/Dockerfile .

docker-windows:
	docker buildx build --build-arg='BUILD_TAGS=mainnet,experimental' -t grant-node:0.7.5-windows-binary -f docker/Dockerfile.window-build .

docker-linux:
	docker buildx build --build-arg='BUILD_TAGS=mainnet,experimental' -t grant-node:0.7.5-linux-binary -f docker/Dockerfile.linux-build .